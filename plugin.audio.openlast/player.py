...
class LovedTracksPlayer(xbmc.Player):
    ...
    def init(self, username):
        ...
        self.tracks = self.loadAllLovedTracks(self.username)
...
class TrackLibraryPlayer(xbmc.Player):
    ...
    def init(self, username, playcount):
        ...
        self.tracks = self.loadAllTracks(self.username)
    ...
    def loadAllTracks(self, username):
        #xbmc.executebuiltin('Notification(%s,%s)' % (SESSION, "Loading loved tracks..."))
        log('Loading tracks')
        loaded = False
        page = 1
        artists = {}
        while (True != loaded):
            resp = self.loadTracks(username, page)

            # for a in resp['toptracks']['@attr']:
            #    xbmc.log(str(a) + ': ' + resp['lovedtracks']['@attr'][str(a)])

            # Track the page number
            totalPages = int(resp['toptracks']['@attr']['totalPages'])
            page += 1
            loaded = (page > totalPages)

            total = int(resp['toptracks']['@attr']['total'])
            #perPage = int(resp['toptracks']['@attr']['perPage'])
            avgPlaycount = self.playcount / total + 1
            ...
            for t in resp['toptracks']['track']:
                if int(t['playcount']) < avgPlaycount:
                    log('...the track\'s playcount exceeds the average: avg=' + str(avgPlaycount))
                    page = totalPages
                    loaded = True
                    break
                # xbmc.log(json.dumps(t).encode('utf-8'))
    ...
    def generatePlaylist(self, username):
        artists = self.loadAllTracks(username)
        #xbmc.executebuiltin('Notification(%s,%s)' % (SESSION, "Finding local songs..."))
        log('Finding local songs...')
        totalTracks = 0
        items = []
